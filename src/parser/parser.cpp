...
std::unique_ptr<Node> parseConditional(const std::vector<lexer::Token>& tokens, int& pos) {
    ...
    if (pos < tokens.size() &&
        (tokens[pos].type == lexer::TokenType::EQUALS || tokens[pos].type == lexer::TokenType::GREATERTHAN ||
...
std::unique_ptr<Node> parseExpression(const std::vector<lexer::Token>& tokens, int& pos) {
    ...
    while (pos < tokens.size() &&
           (tokens[pos].type == lexer::TokenType::PLUS || tokens[pos].type == lexer::TokenType::SUBTRACT)) {
        lexer::Token token = tokens[pos++];
...
std::unique_ptr<Node> parseTerm(const std::vector<lexer::Token>& tokens, int& pos) {
    ...
    while (pos < tokens.size() &&
           (tokens[pos].type == lexer::TokenType::MULTIPLY || tokens[pos].type == lexer::TokenType::DIVIDE)) {
        lexer::Token token = tokens[pos++];
...
void printAST(const std::unique_ptr<Node>& node, int indent) {
    ...
    std::cout << indentation << "NodeType: " << static_cast<int>(node->type) << ", Token: " << node->token.value
              << ", Value: " << node->value << "\n";
